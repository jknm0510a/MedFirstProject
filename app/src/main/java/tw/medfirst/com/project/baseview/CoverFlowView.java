...
import tw.medfirst.com.project.adapter.BaseCoverFlowAdapter;
import tw.medfirst.com.project.adapter.CustomCoverFlowAdapter;
import tw.medfirst.com.project.baseunit.BitmapUtils;
 ...
 * 
 */
public class CoverFlowView<T extends BaseCoverFlowAdapter> extends View {
    ...
    private List<RectF> mTouchRect2;
    ...
    protected CoverFlowListener<T> mCoverFlowListener;
...
//        final int scaleWidth = mWidth * (int)childHeightScale;
        ...
        if (offset <= 0) {
            ...
            translateX += Math.abs(offset) * 100;
        }else {
...
//        mDrawChildPaint.setAlpha((int) alpha);
        ...
        mChildTransfromer.postScale(childHeightScale, childHeightScale, 0, 0);
        ...
        recfAllViews(position, child);


        mReflectionTransfromer.preTranslate(0, -(childTotalHeight >> 1));
        mReflectionTransfromer.postScale(childHeightScale, childHeightScale, 0, 0);;
    ...
    private void recfAllViews(int position, Bitmap child) {
        int actuallyPosition = getActuallyPosition(position);
        float[] values = new float[9];
        mChildTransfromer.getValues(values);
//        Log.e("BBB", Integer.toString(actuallyPosition));
//        RectF a = new RectF();
        mTouchRect2.get(actuallyPosition).left = values[2];
        mTouchRect2.get(actuallyPosition).top = values[5];
        mTouchRect2.get(actuallyPosition).right = mTouchRect2.get(getActuallyPosition(position)).left + child.getWidth() * values[0];
        mTouchRect2.get(actuallyPosition).bottom = mTouchRect2.get(getActuallyPosition(position)).top + child.getHeight() * values[0];
...
//                    " touch ==>" + event.getX() + " , " + event.getY());
            if( mTouchRect != null && mCoverFlowListener != null && topImageClickEnable
                    && !mLongClickTriggled) {
                final int actuallyPosition = mTopImageIndex;
                if(mTouchRect2.get(actuallyPosition).contains(event.getX(), event.getY())) {
                    mCoverFlowListener.topImageClicked(this, actuallyPosition);
                }else{
                    final int totalImage = VISIBLE_VIEWS * 2 + 1;
                        for(int i = 1; i <= VISIBLE_VIEWS; i++){
                            int preNum = (actuallyPosition + (totalImage - i)) % totalImage;
                            int postNum = (actuallyPosition + i) % totalImage;
                            if(mTouchRect2.get(preNum).contains(event.getX(), event.getY())) {
                                startAnimation(-(i + 1) * 2);
                                break;
                            }else if(mTouchRect2.get(postNum).contains(event.getX(), event.getY())) {
                                startAnimation((i + 1) * 2);
                                break;
                            }
                        }
...
//                if(mTouchRect2.get(actuallyPosition).contains(event.getX(), event.getY())){
//                    mCoverFlowListener.topImageClicked(this, actuallyPosition);
//                }else if(mTouchRect2.get((actuallyPosition + 4) % 5).contains(event.getX(), event.getY())){
//                    startAnimation(-4);
//                    Log.e("onclick: ", Integer.toString((actuallyPosition + 4) % 5));
//                }else if(mTouchRect2.get((actuallyPosition + 1) % 5).contains(event.getX(), event.getY())){
//                    startAnimation(4);
//                    Log.e("onclick: ", Integer.toString((actuallyPosition + 1) % 5));
//                }else if(mTouchRect2.get((actuallyPosition + 3) % 5).contains(event.getX(), event.getY())){
//                    startAnimation(-6);
//                    Log.e("onclick: ", Integer.toString((actuallyPosition + 3) % 5));
//                }else if(mTouchRect2.get((actuallyPosition + 2) % 5).contains(event.getX(), event.getY())){
//                    startAnimation(6);
//                    Log.e("onclick: ", Integer.toString((actuallyPosition + 2) % 5));
//                }
//                    Log.e(VIEW_LOG_TAG,
//                            " touch ==>" + event.getX() + " , " + event.getY());
    ...
    public static interface CoverFlowListener<V extends BaseCoverFlowAdapter> {
