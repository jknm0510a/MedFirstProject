...
public class CoverFlowView<T extends CoverFlowAdapter> extends View {
    ...
    private static final int TOUCH_MINIMUM_MOVE = 10;
...
//            Log.e("AAAAAAA", Integer.toString(mItemCount) + "  "  + Integer.toString(VISIBLE_VIEWS << 1) + 1);
...
//            }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int visibleCount = mAdapter.getCount();//(VISIBLE_VIEWS << 1) + 1;
	...
	if(position != 0){
        ...
        if (Math.round(offset) < 0)
            translateX = (((float) (leftSpace / VISIBLE_VIEWS))
                    * (VISIBLE_VIEWS + offset) + mCoverFlowPadding.left)
                    - (((VISIBLE_VIEWS) - Math.abs(offset)) * 25);
        else if(Math.round(offset) == 0)
            translateX = ((((float) (leftSpace / VISIBLE_VIEWS))
                    * (VISIBLE_VIEWS + offset) + mCoverFlowPadding.left));
        else
        ...
        mChildTransfromer.postScale(childHeightScale * 0.9f, childHeightScale * 0.9f);
        ...
        getCustomTransformMatrix(mChildTransfromer, mDrawChildPaint, child,
                position, offset);
        float h = (float) ((child.getHeight() * 0.9) - (child.getHeight() * (1 - (0.2 * Math.abs(offset))) * 0.9f)) / 2;
        h = h - Math.abs(offset)*25;
        mChildTransfromer.postTranslate(0, (childTotalHeight >> 1) + h);

        float[] values = new float[9];
        mChildTransfromer.getValues(values);
//        RectF a = new RectF();
        mTouchRect2.get(getActuallyPosition(position)).left = values[2];
...
//        Log.e("imageOnTop", "Left: " + Float.toString(mTouchRect.left) + ", Top: " + Float.toString( mTouchRect.top));
...
//            Log.e("ACTION_DOWN", Float.toString(event.getX()) + ", " + Float.toString(event.getY()) );
    ...
    private void touchMoved(MotionEvent event) {
        ...
        pos /= 2;
//        Log.e("touchMoved", Float.toString(pos));
        if (!mTouchMoved) {
...
//        Log.e("touchMoved", Float.toString(mOffset) + " = " + Float.toString(mStartOffset) + " + " + Float.toString(mTouchStartPos) + " + " + Float.toString(pos));
